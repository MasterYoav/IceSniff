...
| Basic filtering | Yes | Yes | Planned | Shared semantics currently support `protocol=...`, `port=...`, and `host=...`, with well-known-port fallback for fragmented DNS/HTTP/TLS traffic |
...
| Streams and transactions | Partial | Partial | Planned | Implemented as `streams <capture-file> [--filter <expr>]` with client/server roles, directional counts, matched transaction counters, reassembled HTTP transaction counting, and reassembled TLS handshake counting |
