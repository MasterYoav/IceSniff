...
fn packet_addresses(packet: &DecodedPacket) -> (String, String) {
    match &packet.network {
        ...
        Some(NetworkLayerSummary::Ipv6(ipv6)) => {
            (ipv6.source_ip.clone(), ipv6.destination_ip.clone())
        }
    ...
    if let Some(app) = &packet.application {
        ...
        match &packet.transport {
            ...
            None => match &packet.network {
                ...
                Some(NetworkLayerSummary::Ipv6(_)) => "ipv6".to_string(),
...
fn tcp_payload_segment(packet: &DecodedPacket) -> Option<PayloadSegment> {
    let ip_header_len = match &packet.network {
        Some(NetworkLayerSummary::Ipv4(ipv4)) => usize::from(ipv4.header_length),
        Some(NetworkLayerSummary::Ipv6(_)) => 40usize,
        _ => return None,
    };
    let tcp = match &packet.transport {
        Some(TransportLayerSummary::Tcp(tcp)) => tcp,
