...
5. `apps/cli` accepts shared `--filter <expr>` semantics for `list`, `stats`, `conversations`, and `streams`.
...
9. `crates/parser-core` turns loaded packets into list/detail/stats/conversation/stream reports.
